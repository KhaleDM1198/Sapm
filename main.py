...
bot = telebot.TeleBot("6552877762:AAGoZjusMeGCJHe8istpXW8u5AFMhhFFL9c")
running = True
...
def make_call(phone_number, message, msg):
    ...
    try:
        ...
        if response_json.get('message') == 'Sent':
            ...
            print(f"Failed to make call to {phone_number}. error is: {response_json.get('message')}")
            bot.edit_message_text(chat_id=msg.chat.id, message_id=msg.message_id, text=f"Failed to make call to {phone_number}. error is: {response_json.get('message')}")
    ...
    for phone_number in phone_numbers_list:
        ...
        try:
            ...
            if response2.status_code == 200:
                msg = f"Whats Message made to {phone_number} successfully!"
                bot.send_message(chat_id, msg, parse_mode="HTML")
            else:
                msg = f"Failed to make Whats Message to {phone_number}. Status code: {response2.status_code}"
                bot.send_message(chat_id, msg, parse_mode="HTML")
        except requests.exceptions.RequestException as e:
...
def change_send_to(message):
    bot.reply_to(message, "ابعت 1 عشان ترجع لوضع الجروب")
    bot.register_next_step_handler(message, handle_switch)

def handle_switch(message):
    global sendToId
    if message.text == "1":
        sendToId = -1002041695132
        bot.reply_to(message, "Group Now")
    else:
        sendToId = 1098317745
